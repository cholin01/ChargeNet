...
def collate_fn(batch):
    ...
    batch = {prop: batch_stack([mol[prop] for mol in batch]) if prop not in ['filename'] else [mol[prop] for mol in batch]
            for prop in batch[0].keys()}

    to_keep = (batch['charges'].sum(0) > 0)

    for key, prop in batch.items():
        if key not in {'mol_bond_prop', 'edge_index', 'filename'}:
